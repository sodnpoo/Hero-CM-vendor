...
file := $(TARGET_OUT)/etc/AudioPara4.csv
ALL_PREBUILT += $(file)
$(file) : $(LOCAL_PATH)/proprietary/AudioPara4.csv | $(ACP)
	$(transform-prebuilt-to-target)

file := $(TARGET_OUT)/etc/AudioFilter.csv
ALL_PREBUILT += $(file)
$(file) : $(LOCAL_PATH)/proprietary/AudioFilter.csv | $(ACP)
	$(transform-prebuilt-to-target)

file := $(TARGET_OUT)/etc/AudioPreProcess.csv
ALL_PREBUILT += $(file)
$(file) : $(LOCAL_PATH)/proprietary/AudioPreProcess.csv | $(ACP)
	$(transform-prebuilt-to-target)

file := $(TARGET_OUT)/etc/gps.conf
ALL_PREBUILT += $(file)
$(file) : $(LOCAL_PATH)/proprietary/gps.conf | $(ACP)
	$(transform-prebuilt-to-target)
...
include $(CLEAR_VARS)
LOCAL_SRC_FILES := source/hero-keypad.kcm
